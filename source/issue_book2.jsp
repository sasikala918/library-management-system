...
<%
	...
	String msg="";
	String id=session.getAttribute("issue_id").toString();
	String isbn=session.getAttribute("issue_isbn").toString();
	String type=request.getParameter("type");
	if(type.equals("Issue"))
	{
		msg="Book Issued to "+id;
		String str="update books_user set status='2',date='"+date_cur+"' where uid='"+id+"' and ISBN='"+isbn+"'";
		st.executeUpdate(str);
		String decrement="select * from books where ISBN='"+isbn+"'";
		ResultSet rs_decrement=st.executeQuery(decrement);
		int count_book=0;
		while(rs_decrement.next())
		{
			count_book=rs_decrement.getInt("count");
			count_book--;
		}
	...
	{
		msg="Request rejected to "+id;
		String str="update books_user set status='3',date='"+date_cur+"' where uid='"+id+"' and ISBN='"+isbn+"'";
		st.executeUpdate(str);
	}
%>
<%
